package secondtry.reservation.reservations;
...
import secondtry.reservation.reservations.availability.ReservationAvailabilityService;
...
public class ReservationService {
    ...
    public List<Reservation> searchAllByFilter(ReservationSearchFilter filter) {
        ...
        List<ReservationEntity> allEntities = repository.searchAllByFilter(
                ...
                filter.userId(),
                pageable
        ) == null ? new ArrayList<>() : repository.searchAllByFilter(
                filter.roomId(),
                filter.userId(),
                pageable
        );
    ...
    public Reservation createReservation(Reservation reservationToCreate) {
        ...
        if (!reservationToCreate.endDate().isAfter(reservationToCreate.startDate())) {
            throw new IllegalArgumentException("Start date must be minimum 1 day earlier then end date");
        }

        var entityToSave = mapper.toEntity(reservationToCreate);
        entityToSave.setStatus(ReservationStatus.PANDING);
    ...
    public Reservation updateReservation(
        ...
        if (reservationEntity.getStatus() != ReservationStatus.PANDING) {
    ...
    public void cancelReservation(Long id) {
        ...
        if (reservation.getStatus().equals(ReservationStatus.APPROVED)) {
            throw new IllegalStateException("Cannot cancelled reservation. Contact with manager please");
        }

        if (reservation.getStatus().equals(ReservationStatus.CANCELLED)) {
            throw new IllegalStateException("Cannot cancelled reservation. Reservation was already cancelled");
        }
